...
import transport, { COMMON_HEADER_SIZE, LogBlockCommand } from 'hd-transport';
import { createDeferred, Deferred, ERRORS, HardwareErrorCode } from 'hd-shared';
...
export default class ReactNativeBleTransport {
  ...
  /**
   * 获取设备列表
   * 在搜索超过超时时间或设备数量大于 5 台时，返回 ChargerWallet 设备，
   * @returns
   */
  async enumerate() {
  ...
  _monitorCharacteristic(characteristic: Characteristic) {
    ...
    const subscription = characteristic.monitor((error, c) => {
      ...
      try {
        ...
        // console.log('[@chargerwallet/hd-transport-react-native] Received a packet, ', 'buffer: ', data);
        ...
        if (buffer.length - COMMON_HEADER_SIZE >= bufferLength) {
          ...
          // console.log(
          //   '[@chargerwallet/hd-transport-react-native] Received a complete packet of data, resolve Promise, this.runPromise: ',
          //   this.runPromise,
          //   'buffer: ',
          //   value
          // );
